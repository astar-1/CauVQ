...
class GnnNode(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, drop_ratio=0.5, JK="last", residual=True, gnn_name='gin', atom_encode=True):
        '''
            emb_dim (int): node embedding dimensionality
...
class GnnNodeVirtualnode(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, drop_ratio=0.5, JK="last", residual=False, gnn_name='gin', atom_encode=True):
        ...
        for layer in range(num_layer - 1):
            self.mlp_virtualnode_list.append(
                torch.nn.Sequential(torch.nn.Linear(emb_dim, 2 * emb_dim), torch.nn.BatchNorm1d(2 * emb_dim), nn_act, \
    ...
    def forward(self, batched_data):
        ...
        if self.atom_encode:
            h_list = [self.atom_encoder(x.long())]
        else:
            h_list = [x]
